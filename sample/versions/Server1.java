...
public class Server extends UnicastRemoteObject implements IServer {
                ...
                public void run() {
                    while (true) {
                        try {
                            ...
                            int middleScaleOut = bindServer.getScaleOutNum();
                            if (middleScaleOut > 0) {
    ...
    public synchronized int getScaleOutNum() throws RemoteException {
        int dif = requestQueue.size() - activeMiddleVMs;
        // if(dif>0)System.out.println("!!!!!!!!!!! dif: "+dif);
        if (dif > MIDDLE_SCALE_OUT_THRESHOLD) {
            if (activeMiddleVMs == 0) return -1;
            int value = dif/activeMiddleVMs; //(int) Math.ceil(dif/(activeMiddleVMs)); 
            if (value >= 0) {
